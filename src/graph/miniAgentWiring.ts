...
export class MiniAgentWiring {
    ...
    static async wireMiniTopology(
        ...
        try {
            ...
            const addNode = graph.addNode as (name: string, node: unknown) => void;
            const addConditionalEdges = graph.addConditionalEdges as (
                source: string,
                router: unknown,
                destinations: string[],
            ) => void;
            const addEdge = graph.addEdge as (source: string, target: string) => void;
            ...
            // Mini-agent modules may not be available — skip topology wiring
        }
    }
}
